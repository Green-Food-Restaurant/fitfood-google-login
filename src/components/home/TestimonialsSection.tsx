    ...
    <section className="py-20 bg-white dark:bg-gray-800">
      <div className="container mx-auto px-6">
        <motion.div 
          ...
          viewport={{ once: true }}
        >
          <span className="text-green-600 dark:text-green-400 font-medium">DEPOIMENTOS</span>
          <h2 className="text-3xl lg:text-4xl font-bold mt-2 mb-4 dark:text-white">O que nossos clientes dizem</h2>
                ...
                <div>
                  ...
                  <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                      <FaStar 
                        key={i} 
                        className={`text-sm ${i < Math.floor(testimonial.rating) ? '' : 'text-yellow-200 dark:text-yellow-700'}`} 
                      />
                    ))}
                  </div>
                </div>
